			...
			records.forEach((final CSVRecord record) -> {
				...
				for (var i = 0; i < record.getFieldCount(); i++) {
					...
					System.out.printf("  Field %d: %s (null=%s, empty=%s, pos=%d-%d)%n", Integer.valueOf(i),
							field.isNull() ? "NULL" : "'" + field.getValue() + "'", Boolean.valueOf(field.isNull()),
							Boolean.valueOf(field.isEmpty()), Integer.valueOf(field.getStartPosition()),
							Integer.valueOf(field.getEndPosition()));
				}
				System.out.println();
			});
		} catch (final CSVParseException e) {
